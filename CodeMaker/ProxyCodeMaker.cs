...
#$method_params_assignment$#
        ...
        public int Start(string dllName, string servantName,string saveFolder)
        {
            ...
            try
            {
                ...
                string prefixNameSpace = Path.GetFileNameWithoutExtension(dllName);
                int idx = prefixNameSpace.IndexOf(".Protocol", StringComparison.CurrentCultureIgnoreCase);
                if (idx > 0)
                    prefixNameSpace = prefixNameSpace.Substring(0, idx);
                ...
                if (assembly != null)
                {
                    ...
                    if (types != null && types.Length > 0)
                    {
                        ...
                        proxyBody = proxyBody.Replace("#$namespace$#", prefixNameSpace);
                        proxyBody = proxyBody.Replace("#$dll_name$#",Path.GetFileNameWithoutExtension(dllName));
